...
class MusicBot(commands.Cog):
    ...
    async def play_next(self, guild_id):
        if guild_id in guild_queues and guild_queues[guild_id]:
            ...
            if vc is None or not vc.is_connected():
                logger.info(f"Voice client is not connected for guild {guild_id}")
                return
    ...
    async def play(self, interaction: discord.Interaction, url: str):
        ...
        await self.register_song(user_id, user_name, 'Unknown title', url)
    ...
    async def queue(self, interaction: discord.Interaction):
        ...
        if guild_id not in guild_queues or not guild_queues[guild_id]:
            ...
            queue_list = [f"{idx + 1}. {title}" for idx, (_, _, title, _) in enumerate(guild_queues[guild_id])]
            queue_str = "\n".join(queue_list)
    ...
    async def clear(self, interaction: discord.Interaction):
        ...
        if guild_id not in guild_queues or not guild_queues[guild_id]:
            ...
            await interaction.response.send_message("Cleared the song queue.")
